...
def get_installed_sys_packages():
    dpkg_out = subprocess.run(["dpkg", "--list"], stdout=subprocess.PIPE).stdout.decode(
        "utf8"
    )
...
def is_git_config_uptodate():
    cfg = get_git_config()
    return cfg.get("user.email") == user_email and cfg.get("user.name") == user
...
def get_existing_envs():
    ...
    conda_out = subprocess.run(
        [conda_command, "env", "list"], stdout=subprocess.PIPE
    ).stdout.decode("utf8")
...
def task_install_system_packages():
    ...
    return {
        "actions": ["sudo apt update", "sudo apt install -y {}".format(pkg_string)],
...
def task_install_dotnet():
    """Install .NET SDK"""
    return {
        "actions": ["echo 'export DOTNET_CLI_TELEMETRY_OPTOUT=1' >> ${HOME}/.bashrc"],
        "uptodate": [
            "command -v dotnet",
        ],
    }
...
def task_download_mambaforge():
    ...
    return {
        "actions": [
            "wget https://github.com/conda-forge/miniforge/releases/latest/download/Mambaforge-Linux-x86_64.sh -O {}".format(
                mambaforge_setup
            )
        ],
...
def task_install_conda():
    ...
    return {
        "actions": [
            ...
            "echo 'PATH=\"${{PATH}}:{}\"; export PATH' >> ${{HOME}}/.bashrc".format(
                mambaforge_install_folder / "bin"
            ),
        ],
...
def task_setup_docker():
    """Setup docker group"""
    return {
        "actions": ["sudo groupadd -f docker", "sudo usermod -aG docker $USER"],
...
def task_update_repository():
    ...
    for repo_info in get_repos_envs():
        yield {
            ...
            "uptodate": [
                False
            ],  # always pull as we don't know anything about remote source
        }
    ...
    for repo_info in get_repos_envs():
        if repo_info.env:
            yield {
                "name": repo_info.env,
                "actions": [
                    "{} create -y -n {} python={}".format(
                        conda_command, repo_info.env, conda_env_python_version
                    )
                ],
                "task_dep": [
                    "install_conda",
    ...
    for repo_info in get_repos_envs():
        if repo_info.env:
            yield {
                "name": repo_info.env,
                "actions": [
                    "{} pip install -e {} -U".format(
                        repo_info.conda_run, repo_info.folder
                    ),
                    "{} pip install -r {} -U".format(
                        repo_info.conda_run, repo_info.requirements_dev
                    ),
                ],
...
def task_update_repo_env_list():
    """Update config file containing repositories by querying github API
    for new projects."""
    return {"actions": [update_repo_env_list], "uptodate": [False]}
